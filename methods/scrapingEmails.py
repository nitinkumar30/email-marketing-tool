...
def scrap_mails(url, mails_bundle):
    ...
    # Use a regular expression to find all the email ids in the HTML content
    ...
    for tag in email_tags:
        if tag.has_attr('href') and 'mailto:' in tag['href']:
            ...
            matches = re.findall(email_pattern, str(tag))
            for match in matches:
                emails.add(match)

    # Print the list of email ids found
    print('Email list under scrapEmails method', emails)
